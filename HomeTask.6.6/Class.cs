...
namespace HomeTask._6._6
{
    public struct Class11
    {
        ...
        public static string AddNewID()
        {
            ...
            //                $"#{list1.Age}#{list1.Height}#{list1.BirthDay}#{list1.BirthCity}\n";
        ...
        public static void SortFileDate(char keySort)
        {
            ...
            if (keySort == key1)
            {
                while (nose != initArray.Length)
                {
                    for (int i = 0; i < tail; i++)
                    {
                        if (Convert.ToDateTime(initArray[nose][colomn]) >= Convert.ToDateTime(initArray[i][colomn])) continue;
                        else
                        {
                            string[] transition = initArray[nose];
                            initArray[nose] = initArray[i];
